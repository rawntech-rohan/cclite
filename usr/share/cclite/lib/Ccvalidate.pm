...
sub validate_user {
    ...
    my ( $error, $fromuserref ) =
      get_where( $class, $db, 'om_users', 'userMobile', $$fieldsref{userMobile},
        $token, $offset, $limit );
    ...
    # not accepted if username exists already or email exists already
    # now updated for blank 'order by' string after sql string
    # ugly needs doing with count, really

    my ( $error1, $fromuserref1 ) =
      get_where( $class, $db, 'om_users', 'userLogin', $$fieldsref{userLogin},
        $token, $offset, $limit );
    my ( $error2, $fromuserref2 ) =
      get_where( $class, $db, 'om_users', 'userEmail', $$fieldsref{userEmail},
        $token, $offset, $limit );
    ...
    if ( !length($error1) && !length($error2) ) {
        ...
        ###$log->debug("nameexists: $nameexists") ;
        ###$log->debug("emailexist: $emailexists") ;
    }
    ...
    my $x = join( "|", @status );
    ###$log->debug("in validate status is: $x") ;
...
sub validate_partner {
    ...
    if ( $$fieldsref{'type'} eq 'local' ) {
        ...
        ###$log->debug("regex for validate_partner $regex");
...
sub validate_transaction {
    ...
    ###$log->debug("in validate transaction") ;
...
sub _validate_address {
    ...
    if ( $$fieldsref{userStreet} !~ /[a-zA-z](\w+)\s+(\w+)/ ) {
